...
pub use prompt::{PromptDetector, CommandCompletionDetector, ShellType, utils as prompt_utils};
...
use crate::models::command_block::ExecutionStatus;
...
impl Terminal {
    ...
    pub async fn process_output(&mut self, data: &[u8], stream_type: StreamType) -> Result<Vec<OutputLine>> {
        ...
        if self.completion_detector.is_command_complete(&lines) {
            ...
            if let Some(command) = self.state.get_last_command() {
...
pub mod utils {
    ...
    pub fn create_default_session() -> TerminalSession {
        ...
        let shell_type = match shell_path.as_str() {
            ...
            _ => ShellType::Other(shell_path.clone()),
        };
